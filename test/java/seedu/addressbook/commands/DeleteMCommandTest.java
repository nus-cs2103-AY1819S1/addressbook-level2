...
public class DeleteMCommandTest {
    ...
    public void execute_emptyAddressBook_returnsPersonNotFoundMessage() {
        assertDeletionFailsDueToNoSuchPerson(Arrays.asList(1) , emptyAddressBook, listWithEveryone);
    }

    @Test
    public void execute_noPersonDisplayed_returnsInvalidIndexMessage() {
        assertDeletionFailsDueToInvalidIndex(Arrays.asList(1), addressBook, emptyDisplayList);
    }
    ...
    public void execute_targetPersonNotInAddressBook_returnsPersonNotFoundMessage()
        ...
        assertDeletionFailsDueToNoSuchPerson(Arrays.asList(1), addressBook, listWithPersonNotInAddressBook);
    }

    @Test
    public void execute_invalidIndex_returnsInvalidIndexMessage() {
        assertDeletionFailsDueToInvalidIndex(Arrays.asList(0), addressBook, listWithEveryone);
        assertDeletionFailsDueToInvalidIndex(Arrays.asList(-1), addressBook, listWithEveryone);
        assertDeletionFailsDueToInvalidIndex(Arrays.asList(listWithEveryone.size() + 1), addressBook, listWithEveryone);
    }

    @Test
    public void execute_validIndex_personIsDeleted() throws PersonNotFoundException {
        assertDeletionSuccessful(Arrays.asList(1), addressBook, listWithSurnameDoe);
        assertDeletionSuccessful(Arrays.asList(listWithSurnameDoe.size()), addressBook, listWithSurnameDoe);
     ...
     *
     * @param targetIndexes of the people that we want to delete
     */
    private DeleteMCommand createDeleteMCommand(List<Integer> targetIndexes, AddressBook addressBook,
                                                                      List<ReadOnlyPerson> displayList) {

        DeleteMCommand command = new DeleteMCommand(targetIndexes);
     ...
     */
    private void assertCommandBehaviour(DeleteMCommand deleteMCommand, String expectedMessage,
                                        AddressBook expectedAddressBook, AddressBook actualAddressBook) {

        CommandResult result = deleteMCommand.execute();
     ...
     */
    private void assertDeletionSuccessful(List<Integer> targetIndexes, AddressBook addressBook,
                                          List<ReadOnlyPerson> displayList) throws PersonNotFoundException {

        ReadOnlyPerson targetPerson = displayList.get(targetIndexes.get(0) - TextUi.DISPLAYED_INDEX_OFFSET);
        ...
        DeleteMCommand command = createDeleteMCommand(targetIndexes, actualAddressBook, displayList);
