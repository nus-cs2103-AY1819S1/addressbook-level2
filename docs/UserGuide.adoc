...
== Finding all persons containing any keyword in their tag: `tag`

Finds persons whose names contain any of the specified keywords(case-sensitive). +
Format: `find KEYWORD [MORE_KEYWORDS]`
...
* `tag Friends` +
Returns Any person having the tag `Friends` but not `friends`.

* `tag classmates friends family` +
Returns Any person having tags `classmates`, `friends`, or `family`.
