...
== Finding all persons containing any keyword in their name/address/email/phone: `find`

Finds persons whose names/address/email/phone contain any of the given keywords. +
Format: `find KEYWORD [MORE_KEYWORDS]`

[NOTE]
====
The search is case sensitive, the order of the keywords does not matter, the name, address, email and phone number are
searched, and persons matching at least one keyword will be returned (i.e. `OR` search).
Keep in mind that the search requires exact keyword for email id and phone number since they are single words.
====
...
*  `find Clementi` +
Returns Any person having `Clementi` in their address.

*  `find ab@gmail.com` +
Returns Any person having their email id equal to `ab@gmail.com`
