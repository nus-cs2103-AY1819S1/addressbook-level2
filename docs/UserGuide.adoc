...
== Sort all persons by name and then by phone number: `sort`

Sorts persons by name and if the name matches, then by phone number. +
Format: `sort`

[NOTE]
====
Two records having the same name and phone number are considered as duplicate records.
====
