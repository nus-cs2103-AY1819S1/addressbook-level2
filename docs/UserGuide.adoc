...
Edits a person in the address book. If the person cannot be found, the person is added instead. +
Format: `edit NAME [p]p/PHONE_NUMBER [p]e/EMAIL [p]a/ADDRESS [t/TAG]...`

****
This command has the same syntax as the `add` command, save for the command name.
****
