...
== Finding all persons are similar to any keyword in their name: `fuzzyFind`

Finds persons whose names are similar to any of the given keywords. +
Format: `find KEYWORD [MORE_KEYWORDS]`

[NOTE]
====
The 'similar' means the https://en.wikipedia.org/wiki/Levenshtein_distance[levenshtein distance] between two words is less than or equal to 2.
====

Examples:

* `fuzzyFind Betsy Tim John` +
Returns Any person having names similar to `Betsy`, `Tim`, or `John`.
