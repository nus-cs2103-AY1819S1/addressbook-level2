...
== Deleting every person in the list : `deleteall`
Deletes every person in the last viewed list. Irreversible. +
Format: `deleteall`

****
Deletes every person in the last viewed list. The list refers to the list
displayed from the last command such as `list` or `find`.
****
...
* `list` +
`deleteall` +
Deletes every person displayed in the list. In this case it functions like `clear`.

* `find Jack` +
`deleteall` +
Deletes all persons in the results of the `find` command.
