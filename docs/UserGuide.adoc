...
== Finding person(s) with a given phone number: `num`

Finds person(s) whose phone contains the given 8-digit number. +
Format: `num 8-DIGIT-NUMBER`

[NOTE]
====
The search is sensitive to having exactly 8digits as per info supplied by the user when adding.
Multiple people with the same contact number will be listed.
====

Examples:
* `find 12345678` +
Returns anyone with the phone number 12345678.
